...
#include "LidarSlam/KDTreeTrajectoryAdaptor.h"
...
void Slam::Reset(bool resetLog)
{
  ...
  this->LastReliableExtPoseTime = -1.0;
...
void Slam::SetVerbosity(int verbosity)
{
  ...
  // Set verbosity for keypoints extraction
  for (const auto& kv : this->KeyPointsExtractors)
    kv.second->SetVerbosity(verbosity);
}
...
void Slam::AddFrames(const std::vector<PointCloud::Ptr>& frames)
{
  ...
  if (this->IsKeyFrame || this->IsPoseJumped)
  {
    if (this->PGOPendingFramesAfterPoseJump > 0)
      this->PGOPendingFramesAfterPoseJump--;
  ...
  // Trigger pose graph optimization when a external sensor drop was detected
  if (this->EnableAutoCorrectExtPose && this->UsePGOConstraints[PGOConstraint::EXT_POSE] && this->PoseHasData())
  {
    if (this->LastReliableExtPoseTime < 0)
      this->LastReliableExtPoseTime = this->CurrentTime;
    // Check current external sensor reliability
    bool extSensorReliable = this->PoseManager->IsMeasurementReliable(this->CurrentTime);
    // Trigger graph optimization if external sensor has just returned after being unavailable
    // Do not optimize graph if there is also a time jump between lidar frames because the current pose is
    // discontinuous and will be removed later
    if (!this->TimeJump && this->CurrentTime - this->LastReliableExtPoseTime > this->ExtPoseDropThreshold && extSensorReliable)
    {
      PRINT_WARNING("External sensor recovered after drop. Triggering pose graph optimization.")
      this->OptimizeGraph();
      this->PGOHasBeenTriggered = true;
    }
    // Update last valid external sensor time
    if (extSensorReliable)
      this->LastReliableExtPoseTime = this->CurrentTime;
  }
...
void Slam::ComputeSensorConstraints()
{
  ...
  if (this->CameraCanBeUsedLocally())
  {
    ...
    PointCloud::Ptr aggregatedFrames = this->AggregateFrames(this->CurrentFrames, false, true, true);
    ...
    for (auto& state : this->LogStates)
    {
      ...
      for (auto k : this->UsableKeypoints)
      {
        ...
        *undistortedKeypoints = *rawKeypoints;
        ...
        keypoints->header = Utils::BuildPclHeader(Utils::SecToPclStamp(state.Time), this->BaseFrameId, state.Index);
...
bool Slam::OptimizeGraph()
{
  ...
  if (this->PGOPendingFramesAfterPoseJump > 0)
  {
    PRINT_WARNING("Pose has been jumped recently, graph cannot be optimized");
    return false;
  }

  if ((!this->UsePGOConstraints[PGOConstraint::LANDMARK]     || !this->LmHasData())  &&
      (!this->UsePGOConstraints[PGOConstraint::GPS]          || !this->GpsHasData()) &&
      (!this->UsePGOConstraints[PGOConstraint::MANUAL_POSITION] || !this->ManualPositionHasData()) &&
  ...
  graphManager.SetFixFirst(this->FixFirst);
  graphManager.SetFixLast(this->FixLast);
  ...
  // Look for manual absolute position constraints
  if (this->UsePGOConstraints[PGOConstraint::MANUAL_POSITION] && this->ManualPositionHasData())
  {
    graphManager.AddExternalSensor(Eigen::Isometry3d::Identity(), ExternalSensor::MANUAL_POSITION);

    for (const auto& m : this->ManualPositionAnchors)
    {
      // Find closest logged state to the manual anchor time
      auto chosenIt = this->GetClosestState(m.Time);

      // If we use fixed pose, use current state's pose as target; else use provided position target.
      Eigen::Isometry3d targetPose = m.UseFixedPose ? Eigen::Isometry3d(chosenIt->Isometry)
                                                    : Eigen::Isometry3d(m.Pose);

      graphManager.AddManualPoseConstraint(chosenIt->Index, targetPose, m.Covariance, m.UseFixedPose);
      externalConstraint = true;
    }
  }
  ...
  if (this->UsePGOConstraints[PGOConstraint::EXT_POSE] && this->PoseHasData())
  {
    ...
    for (auto itState = startIt; itState != this->LogStates.end(); ++itState)
    {
      ...
      ++idx;
      if (std::abs(poseSynchMeasure.Time + this->SensorTimeOffset - itState->Time) > 1e-6)
        continue;

      // Skip unreliable pose
      if (this->PoseManager->GetCovarianceRotation() && !this->PoseManager->IsMeasurementReliable(poseSynchMeasure))
        continue;
  ...
  // Call ResetTrajWithTworldInit unless MANUAL_POSITION constraint is enabled

  // We assume that the PGO with manual position constraints is performed in the external sensor reference frame.
  // In practice, manual anchors are identified by comparing SLAM results with an external georeferenced map
  // expressed in the GNSS reference frame.
  if (!this->UsePGOConstraints[PGOConstraint::MANUAL_POSITION] && !this->FixFirst)
...
//-----------------------------------------------------------------------------
void Slam::JumpPose(const Eigen::Isometry3d& pose, bool resetKeypoints)
{
  // Set current pose
  this->Tworld = pose;
  ...
  // Notify pose has been jumped
  this->IsPoseJumped = true;
  this->PGOPendingFramesAfterPoseJump = POSE_JUMP_PENDING_FRAMES;
}
...
//-----------------------------------------------------------------------------
void Slam::SaveMapsToPCD(const std::string& filePrefix, PCDFormat pcdFormat, bool filtered, bool fixed)
{
  ...
  // Do not update maps if initial maps have been loaded
  if (this->LoggingTimeout > this->GetVoxelGridDecayingThreshold()
      && this->GetVoxelGridDecayingThreshold() > 0
      && !this->HasInitialMaps)
    this->UpdateMaps(true);
  ...
  for (auto k : this->UsableKeypoints)
  {
    auto pointcloud = this->GetMap(k, filtered);
    // Label points as fixed
    if (fixed)
    {
      for (auto& pt : *pointcloud)
        pt.label = static_cast<int>(LidarPointLabel::FIXED);
    }
    savePointCloudToPCD(filePrefix + "_" + Utils::Plural(KeypointTypeNames.at(k)) + ".pcd",  *pointcloud,  pcdFormat, true);
  }
  IF_VERBOSE(3, Utils::Timer::StopAndDisplay("Keypoints maps saving to PCD"));
}

//-----------------------------------------------------------------------------
void Slam::LoadMapsFromPCD(const std::string& filePrefix, bool resetMaps, bool keepLabel, double mapTime)
{
  IF_VERBOSE(3, Utils::Timer::Init("Keypoints maps loading from PCD"));
  this->HasInitialMaps = false;
  ...
  for (auto k : this->UsableKeypoints)
  {
    ...
    if (pcl::io::loadPCDFile(path, *keypoints) == 0)
    {
      ...
      // If mapping mode is NONE or ADD_KPTS_TO_FIXED_MAP and the keepLabel is not enabled, the first map points are fixed,
      // else, the initial map points can be updated and the point labels are kept
      bool fixedMap = (this->MapUpdate == MappingMode::NONE || this->MapUpdate == MappingMode::ADD_KPTS_TO_FIXED_MAP) && !keepLabel;
      this->LocalMaps[k]->Add(keypoints, fixedMap);
      this->HasInitialMaps = true;
    }
  }
...
//-----------------------------------------------------------------------------
void Slam::LoadMapsFromPointcloud(const std::unordered_map<Keypoint, PointCloud::Ptr>& keypointMaps, bool resetMaps, bool keepLabel, double mapTime)
{
  IF_VERBOSE(3, Utils::Timer::Init("Keypoints maps loading from pointcloud"));
  this->HasInitialMaps = false;
  if (keypointMaps.empty())
  {
    PRINT_WARNING("Keypoint maps are empty, stop loading maps")
    IF_VERBOSE(3, Utils::Timer::StopAndDisplay("Keypoints maps loading from pointcloud"));
    return;
  }
  // In most of the cases, we would like to reset SLAM internal maps before
  // loading new maps to avoid conflicts.
  ...
  for (const auto& k : this->UsableKeypoints)
  {
    auto it = keypointMaps.find(k);
    if (it == keypointMaps.end() || !it->second)
      continue;
    auto keypoints = it->second;
    if (mapTime > 0)
    {
      for (auto& pt: *keypoints)
        pt.time = mapTime;
...
Slam::PointCloud::Ptr Slam::GetRegisteredFrame()
{
  ...
  if (this->RegisteredFrame->header.stamp != this->CurrentFrames[0]->header.stamp)
    this->RegisteredFrame = this->AggregateFrames(this->CurrentFrames, true, true, true);
...
bool Slam::CheckFrames(const std::vector<PointCloud::Ptr>& frames)
{
  ...
  // Skip frames if the timestamp is older than previous ones
  if (!this->CurrentFrames.empty() && frames[0]->header.stamp < this->CurrentFrames.front()->header.stamp)
  {
    PRINT_ERROR("SLAM frames have an older timestamp (" << std::fixed << std::setprecision(3) << Utils::PclStampToSec(frames[0]->header.stamp)
                << ") than previous ones(" << Utils::PclStampToSec(this->CurrentFrames.front()->header.stamp) << std::scientific
                << "): frames ignored.");
    return false;
  }
  ...
  for (unsigned int i = 0; i < frames.size(); ++i)
  {
    ...
    if ((this->PreviousFramesSeq[i] > 0) && (droppedFrames > 0))
      PRINT_WARNING(droppedFrames << " frame(s)" << (frames.size() > 1 ? " from LiDAR device " + frames[i]->header.frame_id : "") << " were dropped by SLAM\n");
    this->PreviousFramesSeq[i] = frames[i]->header.seq;
  }

  // Check frame timestamp skipping
  unsigned int frameSize = std::min(frames.size(), this->CurrentFrames.size());
  this->TimeJump = false;
  for (unsigned int i = 0; i < frameSize; ++i)
  {
    double timeInterval = Utils::PclStampToSec(frames[i]->header.stamp) - Utils::PclStampToSec(this->CurrentFrames[i]->header.stamp);
    if (timeInterval > 2.0 * this->SensorTimeThreshold)
    {
      PRINT_WARNING("There is a " << timeInterval << " seconds time gap between consecutive frames " << (frames.size() > 1 ? "from LiDAR device " + frames[i]->header.frame_id : ""));
      this->TimeJump = true;
    }
  }
...
//-----------------------------------------------------------------------------
bool Slam::ComputeOdomToExtRefTransform(Eigen::Isometry3d& transform, double time)
{
  // Compute the transform from slam odom frame to external sensor frame
  // LogStates should not be empty
  if (this->LogStates.empty())
  {
    PRINT_WARNING("No LogStates: cannot find any synchronized pose")
    return false;
  }
  std::list<LidarState>::const_iterator itSt = this->LogStates.cend();
  ExternalSensors::PoseMeasurement synchMeas; // Virtual measure with synchronized timestamp and calibration applied
  bool synchPoseFound = false;
  if (time < 0)
  {
    // Search from first to last logstates
    for (auto it = this->LogStates.begin(); it != this->LogStates.end(); ++it)
    {
      synchPoseFound = this->PoseManager->ComputeSynchronizedMeasureBase(it->Time, synchMeas);
      if (synchPoseFound)
      {
        itSt = it;
        break;
  ...
  {
    itSt = this->GetClosestState(time);
    synchPoseFound = this->PoseManager->ComputeSynchronizedMeasureBase(itSt->Time, synchMeas);
  }

  if (synchPoseFound)
  {
    transform = synchMeas.Pose * itSt->Isometry.inverse();
    PRINT_INFO("The odom to external pose reference transform is computed to : \n" << transform.matrix());
  }
  else
  {
    PRINT_WARNING("Cannot find any synchronized pose for LogStates : SLAM trajectory and maps are not updated");
  }
  return synchPoseFound;
}
...
bool Slam::MoveOdomToExtPosesRefFrame(double time)
{
  ...
  else
  {
    ...
    Eigen::Isometry3d update = Eigen::Isometry3d::Identity();
    if (!this->ComputeOdomToExtRefTransform(update, time))
    {
      return false;
    }
    for (auto& state: this->LogStates)
    ...
    SET_COUT_FIXED_PRECISION(3);
    PRINT_VERBOSE(1, " The offset added onto slam trajectory:\n"
                     " translation = [" << update.translation().transpose() << "] m\n"
                     " rotation    = [" << Utils::Rad2Deg(Utils::RotationMatrixToRPY(update.linear())).transpose() << "] °");
    RESET_COUT_FIXED_PRECISION;
  }
...
void Slam::ComputeEgoMotion()
{
       ...
       this->EgoMotion == EgoMotionMode::EXTERNAL_OR_MOTION_EXTRAPOLATION))
  {
    if (this->PoseHasData())
    {
      // Jump pose to synchronized external pose at current time when there is a time gap between frames
      if (this->TimeJump && this->LogStates.size() > 1)
      {
        ...
        {
          bool offsetValid = !this->PoseManager->GetOffset().matrix().isIdentity() ||
                              this->PoseManager->UpdateOffset(this->LogStates);
          if (offsetValid)
          {
            this->JumpPose(this->PoseManager->GetOffset() * synchPoseMeas.Pose, false);
            externalAvailable = true;
            PRINT_WARNING("Jump pose to external pose")
          }
          else
            PRINT_WARNING("Cannot compute external pose offset: skipping jump, falling back to motion extrapolation")
        }
      }
      else
      {
        ...
        if (this->PoseManager->ComputeSynchronizedMeasureBase(this->LogStates.back().Time, synchPreviousPoseMeas))
        {
          ...
          if (this->PoseManager->ComputeSynchronizedMeasureBase(this->CurrentTime, synchPoseMeas))
          {
            ...
            // Do not trust external ego motion if the motion is too strong
            externalAvailable = this->Trelative.translation().norm() > this->EgoMotionLimit ? false : true;
...
void Slam::Localization()
{
  ...
  else
  {
    ...
    {
      // No OMP here: pose manager is not fully thread-safe and concurrent access would cause race conditions
      for (auto k : this->UsableKeypoints)
      {
...
void Slam::LogCurrentFrameState()
{
  ...
  float defaultAngleError = Utils::Deg2Rad(1.f); // 0.0175 rad
  if (!Utils::isCovarianceValid(state.Covariance))
    state.Covariance = Utils::CreateDefaultCovariance(defaultPositionError, defaultAngleError); //1cm, 1°
...
//-----------------------------------------------------------------------------
void Slam::AutoDetectLoopClosure()
{
  if (this->LogStates.size() < 2)
  {
  ...
  itQueryState = this->FetchStateIndex(itQueryState, 2 * this->LoopParams.GapLength);
  auto itRevisitedState = this->LogStates.begin();
  Eigen::Isometry3d loopTransform = Eigen::Isometry3d::Identity();
  while (itQueryState != std::prev(this->LogStates.end()))
  {
    if (this->DetectLoopWithNearestPose(itQueryState, itRevisitedState, loopTransform))
    {
      this->LoopDetections.emplace_back(itQueryState->Index, itRevisitedState->Index, itQueryState->Time);
      this->LoopDetections.back().Transform = loopTransform;
    }
    itQueryState = this->FetchStateIndex(itQueryState, this->LoopParams.SampleStep);
  }
}
...
bool Slam::DetectLoopClosureIndices(LoopClosure::LoopInfo& loop)
{
  ...
  switch (this->LoopParams.Detector)
  {
    ...
    case LoopClosureDetector::NEAREST_POSE:
    {
      // Automatic detection of loop closure by nearest pose search in the trajectory
      detectionValid = this->DetectLoopWithNearestPose(itQueryState, itRevisitedState, loopTransform);
      break;
    }
                                ...
                                Eigen::Isometry3d& transform)
{
  ...
  while (itSt->Index < static_cast<unsigned int>(lastFrameIdx) && itSt != this->LogStates.end())
  {
    ...
    float overlapEstimation = this->ComputeTransformWithTeaser(queryPoints, candidatePoints, candidateSubMaps, candidateLCTransform);
  ...
  if (overlap >= this->LoopParams.EvaluationOverlapThreshold)
  {
    transform = queryPose;
...
#ifdef USE_TEASERPP
//-----------------------------------------------------------------------------
float Slam::ComputeTransformWithTeaser(const PointCloud::Ptr queryPoints,
                                       const PointCloud::Ptr candidatePoints,
                                       Maps& candidateMaps,
                                       Eigen::Isometry3d& transform)
{
  ...
  transform.translation() = solution.translation;
  transform.linear() = solution.rotation;
  ...
  pcl::transformPointCloud(*queryPoints, *queryPointsTransformed, transform.matrix().cast<float>());

  // Build kdtree for candidate sub maps
  for (auto k : this->UsableKeypoints)
  {
    // Build kdtree on all grid points
    candidateMaps[k]->BuildKdTreeOnAllPts();
  }
  // Compute LCP like estimator
  // (see http://geometry.cs.ucl.ac.uk/projects/2014/super4PCS/ for more info)
  return Confidence::LCPEstimator(queryPointsTransformed, candidateMaps,
...
//-----------------------------------------------------------------------------
bool Slam::DetectLoopWithNearestPose(std::list<LidarState>::iterator& itQueryState,
                                     std::list<LidarState>::iterator& itRevisitedState,
                                     Eigen::Isometry3d& transform)
{
  if (this->LogStates.size() < 2)
  ...
  // Create kd tree of trajectory
  auto itlastCandidateState = this->FetchStateIndex(itQueryState, -this->LoopParams.GapLength);
  std::vector<LidarState*> candidatePoses;
  for (auto it = LogStates.begin(); it != itlastCandidateState; ++it)
  {
    candidatePoses.push_back(&(*it));
  }
  KDTreeTrajectoryAdaptor kdtreeTraj(candidatePoses);

  // Search the nearest pose
  Eigen::Vector3d queryPose = itQueryState->Isometry.translation();
  std::vector<int> knnIdx;
  std::vector<float> knnDist;
  auto numNeighbor = kdtreeTraj.KnnSearch(queryPose, 1, knnIdx, knnDist);
  if (numNeighbor < 1 || knnDist[0] > this->LoopParams.EvaluationDistanceThreshold)
  {
    PRINT_WARNING("No loop closure detected for state #" << itQueryState->Index << " in the current trajectory.\n");
    return false;
  }

  // Get the nearest pose
  itRevisitedState = LogStates.begin();
  std::advance(itRevisitedState, knnIdx[0]);

  // Check overlap with teaser
  #ifdef USE_TEASERPP
  if (!this->LoopParams.EnableOverlapCheck)
    return true;
  // Create query submap and get keypoints in BASE coordinates
  Maps querySubMaps;
  this->InitSubMaps(querySubMaps);
  ...
  // Create revisited submap
  Maps revisitedSubMaps;
  this->InitSubMaps(revisitedSubMaps);
  this->BuildMaps(revisitedSubMaps,
  ...
  // Aggregate candidate keypoints
  PointCloud::Ptr revisitedPoints(new PointCloud);
  for (auto k : this->UsableKeypoints)
    *revisitedPoints += *revisitedSubMaps[k]->Get();

  // TEASER++ registration
  Eigen::Isometry3d candidateLCTransform = Eigen::Isometry3d::Identity();
  float overlap = this->ComputeTransformWithTeaser(queryPoints, revisitedPoints, revisitedSubMaps, transform);
  if (overlap < this->LoopParams.EvaluationOverlapThreshold)
  {
    transform = Eigen::Isometry3d::Identity();
    PRINT_ERROR("No valid loop closure detected for frame #" << itQueryState->Index << ".\n"
                 << "The overlap is " << 100.f * overlap << "% for frame #" << itRevisitedState->Index << " which is too low");
    return false;
  }
  #endif
  PRINT_INFO("A loop closure has been detected for frame #" << itQueryState->Index << " at frame #" << itRevisitedState->Index);
  return true;
}
                                        ...
                                        bool sorted) const
{
  ...
  if (this->PoseManager->UndistortPoints(pc, refTime, startIdx, endIdx, baseToPointsRef, timeOffset, sorted, this->NbThreads))
  {
...
//-----------------------------------------------------------------------------
Slam::PointCloud::Ptr Slam::AggregateFrames(const std::vector<PointCloud::Ptr>& frames, bool worldCoordinates, bool undistort, bool sorted) const
{
...
//-----------------------------------------------------------------------------
void Slam::AddManualPoseMeasurement(double time, const Eigen::Isometry3d& pose, const Eigen::Matrix6d& covariance, bool useFixedPose)
{
  ExternalSensors::ManualPoseMeasurement m;
  m.Time = time;
  m.Pose = pose;
  m.Covariance = covariance;
  m.UseFixedPose = useFixedPose;
  this->ManualPositionAnchors.emplace_back(std::move(m));
}

//-----------------------------------------------------------------------------
void Slam::ClearManualPoseMeasurements()
{
  this->ManualPositionAnchors.clear();
}

//-----------------------------------------------------------------------------
void Slam::ClearManualAnchorsPosition()
{
  auto& v = this->ManualPositionAnchors;
  v.erase(std::remove_if(v.begin(), v.end(), [](const ExternalSensors::ManualPoseMeasurement& m){ return !m.UseFixedPose; }), v.end());
}

//-----------------------------------------------------------------------------
void Slam::ClearManualAnchorsPose()
{
  auto& v = this->ManualPositionAnchors;
  v.erase(std::remove_if(v.begin(), v.end(), [](const ExternalSensors::ManualPoseMeasurement& m){ return m.UseFixedPose; }), v.end());
}
...
//-----------------------------------------------------------------------------
Eigen::Isometry3d Slam::GetExtPose(double stateTime) const
{
  if (this->PoseHasData())
  {
    ExternalSensors::PoseMeasurement synchMeas;
    // Get external pose in external sensor frame NOT in BASE frame
    if (this->PoseManager->ComputeSynchronizedMeasure(stateTime, synchMeas, false)) // do not track time
...
//-----------------------------------------------------------------------------
ExternalSensors::PoseMeasurement Slam::GetLastExtPose() const
{
  if (this->PoseHasData())
  {
    return this->PoseManager->GetMeasures().back();
  }
  return ExternalSensors::PoseMeasurement();
}
...
//-----------------------------------------------------------------------------
bool Slam::GetPoseCovarianceRotation() const
{
  if (this->PoseManager)
    return this->PoseManager->GetCovarianceRotation();
  PRINT_ERROR("Pose sensor has not been set : can't get covariance rotation")
  return false;
}

//-----------------------------------------------------------------------------
void Slam::SetPoseCovarianceRotation(bool rotate)
{
  if (!this->PoseManager)
    this->InitPoseSensor();
  this->PoseManager->SetCovarianceRotation(rotate);
}

//-----------------------------------------------------------------------------
double Slam::GetPoseDistanceThreshold() const
{
  if (this->PoseManager)
    return this->PoseManager->GetDistanceThreshold();
  PRINT_ERROR("Pose sensor has not been set : can't get distance threshold")
  return -1.;
}

//-----------------------------------------------------------------------------
void Slam::SetPoseDistanceThreshold(double dist)
{
  if (!this->PoseManager)
    this->InitPoseSensor();
  this->PoseManager->SetDistanceThreshold(dist);
}

//-----------------------------------------------------------------------------
double Slam::GetPoseMaxStdTranslation() const
{
  if (this->PoseManager)
    return this->PoseManager->GetMaxStdTranslation();
  PRINT_ERROR("Pose sensor has not been set : can't get saturation distance")
  return -1.;
}

//-----------------------------------------------------------------------------
void Slam::SetPoseMaxStdTranslation(double maxStd)
{
  if (!this->PoseManager)
    this->InitPoseSensor();
  this->PoseManager->SetMaxStdTranslation(maxStd);
}

//-----------------------------------------------------------------------------
double Slam::GetPoseMaxStdRotation() const
{
  if (this->PoseManager)
    return this->PoseManager->GetMaxStdRotation();
  PRINT_ERROR("Pose sensor has not been set : can't get saturation distance")
  return -1.;
}

//-----------------------------------------------------------------------------
void Slam::SetPoseMaxStdRotation(double maxStd)
{
  if (!this->PoseManager)
    this->InitPoseSensor();
  this->PoseManager->SetMaxStdRotation(maxStd);
}
...
void Slam::SetKeyPointsExtractor(KeypointExtractorPtr extractor, const std::string& deviceId)
{
  ...
  this->KeyPointsExtractors[deviceId]->SetNbThreads(this->NbThreads);
  this->KeyPointsExtractors[deviceId]->SetVerbosity(this->Verbosity);
}
