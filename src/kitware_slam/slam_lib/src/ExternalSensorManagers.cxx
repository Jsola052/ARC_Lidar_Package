              ...
              other.SensorName)
{
  ...
  this->MaxStdTranslation = other.MaxStdTranslation;
  this->MaxStdRotation = other.MaxStdRotation;
...
bool PoseManager::ComputeConstraint(double lidarTime)
{
  ...
  // If the measurements are not reliable, do not use them to compute constraint
  if (this->CovarianceRotation && (!this->IsMeasurementReliable(synchPrevMeas) || !this->IsMeasurementReliable(synchMeas)))
    return false;
  ...
  // Deduce weight from covariance
  Eigen::Vector6d axisWeights = Eigen::Vector6d::Ones();
  if (this->CovarianceRotation)
  {
    for (int i = 0; i < 6; ++i)
    {
      double stddev = std::sqrt(std::max(1e-6, synchPrevMeas.Covariance(i, i) + synchMeas.Covariance(i, i)));
      axisWeights[i] = 1.0 / stddev;
    }
  }

  this->Residual.Cost = CeresCostFunctions::ExternalPoseResidual::Create(TrelMeas, this->PrevPoseTransform, axisWeights);
...
bool PoseManager::UpdateOffset(const std::list<LidarState>& states)
{
  ...
  PRINT_INFO("External pose offset is updated : \n" << this->Offset.matrix());
  return true;
}

// ---------------------------------------------------------------------------
bool PoseManager::IsMeasurementReliable(double lidarTime)
{
  PoseMeasurement synchMeas; // Virtual measure with synchronized timestamp and calibration applied
  if (!this->ComputeSynchronizedMeasureBase(lidarTime, synchMeas))
    return false;

  if (!synchMeas.Pose.matrix().allFinite())
    return false;

  return this->IsMeasurementReliable(synchMeas);

  return true;
}

// ---------------------------------------------------------------------------
bool PoseManager::IsMeasurementReliable(PoseMeasurement& meas)
{
  // Check translation errors
  double stdX = std::sqrt(meas.Covariance(0, 0));
  double stdY = std::sqrt(meas.Covariance(1, 1));
  double stdZ = std::sqrt(meas.Covariance(2, 2));
  if (stdX > this->MaxStdTranslation || stdY > this->MaxStdTranslation || stdZ > this->MaxStdTranslation)
    return false;
  // Check rotation errors
  double stdRoll  = std::sqrt(meas.Covariance(3, 3)) * 180.0 / M_PI;
  double stdPitch = std::sqrt(meas.Covariance(4, 4)) * 180.0 / M_PI;
  double stdYaw   = std::sqrt(meas.Covariance(5, 5)) * 180.0 / M_PI;
  if (stdRoll > this->MaxStdRotation || stdPitch > this->MaxStdRotation || stdYaw > this->MaxStdRotation)
    return false;
...
// ---------------------------------------------------------------------------
bool PoseManager::UndistortPoints(pcl::PointCloud<LidarPoint>::Ptr pc, double refTime,
                                  int startIdx, int endIdx,
                                  Eigen::Isometry3d baseToPointsRef,
                                  double timeOffset, bool sorted, int nbThread)
{
  ExternalSensors::PoseMeasurement synchPoseMeasCurrent; // Virtual measure with synchronized timestamp and calibration applied

  if (!this->ComputeSynchronizedMeasureBase(refTime, synchPoseMeasCurrent))
  {
  ...
  const int nPts = endIdx - startIdx;
  // Sort points in time order to accelerate synchronized measurement research
  std::vector<int> sortedIndices(nPts);
  std::iota(sortedIndices.begin(), sortedIndices.end(), startIdx);
  if (!sorted)
  {
    std::sort(sortedIndices.begin(), sortedIndices.end(),
              [&](int a, int b) { return pc->at(a).time < pc->at(b).time; });
  }

  // Compute synchronized measures (not parallelizable)
  std::vector<ExternalSensors::PoseMeasurement> synchMeas(nPts); // Virtual measures with synchronized timestamp and calibration applied
  // Compute the synchronized pose for each point in time order to speed up the search of closest measurements
  for (int i = 0; i < nPts; ++i)
  {
    int idxPt = sortedIndices[i];
    this->ComputeSynchronizedMeasureBase(refTime + pc->at(idxPt).time + timeOffset, synchMeas[i]);
  }
  ...
  for (int i = 0; i < nPts; ++i)
  {
    ...
    Eigen::Isometry3d update = invSynchPoseMeasCurrent * synchMeas[i].Pose * baseToPointsRef;
