...
class KDTreeTrajectoryAdaptor
{
public:
  using Pose = LidarState;
  using Trajectory = std::vector<Pose*>;

  using metric_t = typename nanoflann::metric_L2_Simple::traits<float, KDTreeTrajectoryAdaptor>::distance_t;
  using index_t = nanoflann::KDTreeSingleIndexAdaptor<metric_t, KDTreeTrajectoryAdaptor, 3, int>;

  /**
    * \brief Build a Kd-tree from a given trajectory.
    * \param trajectory The trajectory to encode in the kd-tree.
    ...
    */
  KDTreeTrajectoryAdaptor(const Trajectory& trajectory, int leafMaxSize = 16)
  {
    this->Reset(trajectory, leafMaxSize);
  }

  /**
    * \brief Init the Kd-tree from a given trajectory.
    * \param trajectory The trajectory to encode in the kd-tree.
    * \param leafMaxSize The maximum size of a leaf of the tree (refer to
    * https://github.com/jlblancoc/nanoflann#21-kdtreesingleindexadaptorparamsleaf_max_size)
    */
  void Reset(const Trajectory& trajectory, int leafMaxSize = 16)
  {
  ...
  inline size_t KnnSearch(const Eigen::Vector3d& queryPoint, int knearest, std::vector<int>& knnIndices, std::vector<float>& knnSqDistances) const
  {
  ...
  inline const KDTreeTrajectoryAdaptor& derived() const
  {
    return *this;
  }

  inline KDTreeTrajectoryAdaptor& derived()
  {
    return *this;
  }
  ...
  {
    return static_cast<float>(this->Traj[idx]->Isometry.translation()[dim]);
  }
