...
//------------------------------------------------------------------------------
// Create spherical covariance with position and angle error estimations
inline Eigen::Matrix6d CreateCovarianceMatrix(float errX  = 1e-2, float errY  = 1e-2, float errZ = 1e-2,
                                              float errRx = 1e-2, float errRy = 1e-2, float errRz = 1e-2)
{
  Eigen::Matrix6d covariance = Eigen::Matrix6d::Identity();
  covariance(0, 0) = std::max(1e-4, std::pow(errX,  2));
  covariance(1, 1) = std::max(1e-4, std::pow(errY,  2));
  covariance(2, 2) = std::max(1e-4, std::pow(errZ,  2));
  covariance(3, 3) = std::max(1e-4, std::pow(errRx, 2));
  covariance(4, 4) = std::max(1e-4, std::pow(errRy, 2));
  covariance(5, 5) = std::max(1e-4, std::pow(errRz, 2));
...
//-----------------------------------------------------------------------------
inline bool IsPointValid(Eigen::Vector3d& point)
{
...
//-----------------------------------------------------------------------------
inline bool HasNanField(const LidarPoint& point)
{
