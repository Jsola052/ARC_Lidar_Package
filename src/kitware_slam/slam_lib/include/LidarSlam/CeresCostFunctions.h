...
struct ExternalPoseResidual
{
  ...
  ExternalPoseResidual(const Vector6d& extPose,
                       const Eigen::Isometry3d& prevPose,
                       const Vector6d& axisWeights = Vector6d::Ones())
  ...
  bool operator()(const T* const w, T* residual) const
  {
    ...
    // Apply per-axis weight
    for (int i = 0; i < 6; ++i)
      residualVec[i] *= T(this->AxisWeights[i]);
