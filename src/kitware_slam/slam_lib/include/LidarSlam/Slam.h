...
namespace LidarSlam
{
  // Use to pending pose graph optimization after a pose jump
  // PGO needs at least two key frames to construct an edge
  constexpr int POSE_JUMP_PENDING_FRAMES = 5;
...
struct Parameters
{
  ...
  // The overlap threshold to decide whether a candidate frame is the revisited frame of loop closure or not
  double EvaluationOverlapThreshold = 0.6;

  // The distance threshold to decide whether a candidate frame is the revisited frame of loop closure or not
  double EvaluationDistanceThreshold = 1.0;
  ...
  // Boolean to enable the overlap evaluation of loop detection result
  bool EnableOverlapCheck = false;
...
public:
  ...
  void JumpPose(const Eigen::Isometry3d& pose, bool resetKeypoints = true);
  ...
  // If no time is input, the first synchronised measurment found in external poses is used.
  ...
  // PoseInExtRef = transform * slamPose
  bool ComputeOdomToExtRefTransform(Eigen::Isometry3d& transform, double time = -1.);
  ...
  void SaveMapsToPCD(const std::string& filePrefix, PCDFormat pcdFormat = PCDFormat::BINARY_COMPRESSED, bool submap = true, bool fixed = false);

  // Load keypoints maps from disk (and reset SLAM maps)
  void LoadMapsFromPCD(const std::string& filePrefix, bool resetMaps = true, bool keepLabel = false, double mapTime = -1.);
  // Load keypoints maps from pointcloud (and reset SLAM maps)
  void LoadMapsFromPointcloud(const std::unordered_map<Keypoint, PointCloud::Ptr>& keypointMaps, bool resetMaps = true, bool keepLabel = false, double mapTime = -1.);
  ...
  SetMacro(EgoMotionLimit, double)
  GetMacro(EgoMotionLimit, double)
  ...
  GetMacro(PGOHasBeenTriggered, bool)
  SetMacro(PGOHasBeenTriggered, bool)
  ...
  double GetPoseDistanceThreshold() const;
  void SetPoseDistanceThreshold(double dist);

  bool GetPoseCovarianceRotation() const;
  void SetPoseCovarianceRotation(bool rotate);

  float GetPoseSaturationDistance() const;
  void SetPoseSaturationDistance(float dist);

  double GetPoseMaxStdTranslation() const;
  void SetPoseMaxStdTranslation(double maxStd);

  double GetPoseMaxStdRotation() const;
  void SetPoseMaxStdRotation(double maxStd);

  GetMacro(ExtPoseDropThreshold, double)
  SetMacro(ExtPoseDropThreshold, double)

  GetMacro(EnableAutoCorrectExtPose, bool)
  SetMacro(EnableAutoCorrectExtPose, bool)
  ...
  // Get external pose at a specific time in the external sensor reference
  Eigen::Isometry3d GetExtPose(double time) const;

  // Get the last external pose
  ExternalSensors::PoseMeasurement GetLastExtPose() const;
  ...
  // Manual absolute anchors
  void AddManualPoseMeasurement(double time, const Eigen::Isometry3d& pose, const Eigen::Matrix6d& covariance, bool useFixedPose = false);

  void ClearManualPoseMeasurements();

  bool ManualPositionHasData() const { return !this->ManualPositionAnchors.empty(); }

  // Clear all translation-only manual anchors
  void ClearManualAnchorsPosition();
  // Clear all fixed 6-DoF manual anchors (UseFixedPose=true);
  ...
  // Detect automatically loop closures in the entire trajectory
  void AutoDetectLoopClosure();
  ...
  GetStructParamsMacro(Loop, EvaluationOverlapThreshold, double)
  SetStructParamsMacro(Loop, EvaluationOverlapThreshold, double)

  GetStructParamsMacro(Loop, EvaluationDistanceThreshold, double)
  SetStructParamsMacro(Loop, EvaluationDistanceThreshold, double)
  ...
  GetStructParamsMacro(Loop, EnableOverlapCheck, bool)
  SetStructParamsMacro(Loop, EnableOverlapCheck, bool)
...
private:
  ...
  // The ego motion estimation should not be considered valid if Ego motion distance is too large
  // Default value (5 meters) corresponds to 180km/h for LiDAR frequency 10Hz
  double EgoMotionLimit = 5.0;
  ...
  // Indicate if there is a time jump between two consecutive frames
  bool TimeJump = false;
  // Store pose jumping information
  bool IsPoseJumped = false;
  // The PGO should be pended after a pose jump
  int PGOPendingFramesAfterPoseJump = POSE_JUMP_PENDING_FRAMES;
  ...
  // To store whether or not initial maps have been loaded
  bool HasInitialMaps = false;
  ...
  // Manual absolute position anchors
  std::vector<ExternalSensors::ManualPoseMeasurement> ManualPositionAnchors;
  ...
  // Parameter to detect the drop of external sensor
  bool EnableAutoCorrectExtPose = false;
  double LastReliableExtPoseTime = -1.;
  double ExtPoseDropThreshold = 10.; // seconds
  ...
  // Fix first or last frame in pose graph
  bool FixFirst = false;
  bool FixLast = false;

  // Booleans to decide whether to use a pose graph constraint for the optimization
  std::map<PGOConstraint, bool> UsePGOConstraints = {{PGOConstraint::LOOP_CLOSURE, false},
                                                     {PGOConstraint::LANDMARK,     false},
                                                     {PGOConstraint::GPS,          false},
                                                     {PGOConstraint::EXT_POSE,     false},
                                                     {PGOConstraint::MANUAL_POSITION, false}};

  // Parameter to notify PV_wrapping to update the visu
  bool PGOHasBeenTriggered = false;
  ...
  // Detect loop closure by searching the neareset pose in the trajectory
  bool DetectLoopWithNearestPose(std::list<LidarState>::iterator& itQueryState,
  ...
  #ifdef USE_TEASERPP
  // Compute transform between two submaps with teaser
  // Return the overlap estimation value
  float ComputeTransformWithTeaser(const PointCloud::Ptr queryPoints,
  ...
  // By default input pointcloud is not assumed to be in chronological order
  // For better performance, indices are internally sorted by timestamp
  ...
  PointCloud::Ptr AggregateFrames(const std::vector<PointCloud::Ptr>& frames,
                                  bool worldCoordinates = false,
                                  bool undistort = true,
                                  bool sorted = false) const;
