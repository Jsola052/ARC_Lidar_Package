...
// ---------------------------------------------------------------------------
struct ManualPoseMeasurement
{
  double Time = 0.;
  // Absolute pose of the manual anchor in global coordinate system
  Eigen::Isometry3d Pose = Eigen::Isometry3d::Identity();
  // 6x6 covariance on XYZRPY. When UseFixedPose is false, only the 3x3 top-left block
  // (position) is used in the optimizer.
  Eigen::Matrix6d Covariance = Utils::CreateDefaultCovariance();
  // If true, use a fixed pose anchor. If false, use only constrain position (XYZ).
  bool UseFixedPose = false;
};
...
protected:
                                                                                               ...
                                                                                               unsigned int windowSize = 2)
  {
    ...
    if (prevIt == this->Measures.end() || prevIt->Time > lidarTime + this->TimeThreshold)
    {
      prevIt = this->Measures.begin();
    }
    // Go back to find the prevIt before lidarTime if prevIt is slightly after lidarTime
    // It can improve the performance if both egomotion and undistortion mode are set to external
    while (prevIt != Measures.begin() && prevIt->Time > lidarTime)
    {
      --prevIt;
    }
    auto postIt = prevIt;
...
public:
  ...
  GetSensorMacro(MaxStdTranslation, double)
  SetSensorMacro(MaxStdTranslation, double)

  GetSensorMacro(MaxStdRotation, double)
  SetSensorMacro(MaxStdRotation, double)
  ...
  // Check the reliability of a pose measurement
  bool IsMeasurementReliable(PoseMeasurement& meas);
  bool IsMeasurementReliable(double lidarTime);
  ...
  bool UndistortPoints(pcl::PointCloud<LidarPoint>::Ptr pc, double refTime,
                       int startIdx = 0, int endIdx = -1,
                       Eigen::Isometry3d baseToPointsRef = Eigen::Isometry3d::Identity(),
                       double timeOffset = 0, bool sorted = false, int nbThread = 1);
...
protected:
  ...
  // Maximum error on translation to consider a measurement can be used (meters)
  double MaxStdTranslation = 0.1;
  // Maximum error on rotation to consider a measurement can be used (degrees)
  double MaxStdRotation = 1;
