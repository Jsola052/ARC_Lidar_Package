#!/bin/bash
# Helper script to run camera calibration for the back camera on the host laptop.
...
echo "--------------------------------------------------------"
echo "Starting local image decompression for back camera..."
echo "--------------------------------------------------------"
ros2 run image_transport republish compressed raw \
  --ros-args \
  --remap in/compressed:=/camera/back/back_camera/image_raw/compressed \
  --remap out:=/camera/back/back_camera/image_raw_local &
DECOMPRESS_PID=$!
...
echo "1. Hold the checkerboard in front of the back camera."
...
ros2 run camera_calibration cameracalibrator \
  --size 8x6 \
  --square $SQUARE_SIZE \
  image:=/camera/back/back_camera/image_raw_local
